...
img_data = cv2.imread('./6.jpg', 0)  # 以灰度模式加载图片，可以直接写0。
T = 160
plt.title('yuantu')
...
for i in range(0, x):
    for j in range(0, y):
        img_data2[i][j] = 0
# ((dst[i-1][j] > 0) & (dst[i+1][j] < 0)) | ((dst[i-1][j] < 0) & (dst[i+1][j] > 0)) | ((dst[i][j-1] > 0) & (dst[i][j+1] < 0)) | ((dst[i][j+1] > 0) & (dst[i][j-1] < 0)) | ((dst[i-1][j-1] > 0) & (dst[i+1][j+1] < 0)) | ((dst[i-1][j-1] < 0) & (dst[i+1][j+1] > 0)) | ((dst[i+1][j-1] > 0) & (dst[i-1][j+1] < 0)) | ((dst[i+1][j-1] < 0) & (dst[i-1][j+1] > 0))
